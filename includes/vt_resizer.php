	...
	function vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
		...
		if ( $image_src[1] > $width || $image_src[1] <= $width ) {
			...
			if($img_size[0] <= $width || $img_size[1] <= $height) {
				...
				//// IF WE NEED TO MULTIPLY OUR WIDTH
				if($img_size[0] <= $width) { $multiply_w = $width / $img_size[0]; }
				else { $multiply_w = 1; }
				
				//// IF WE NEED TO MULTIPLY OUR HEIGHT
				if($img_size[1] <= $height) { $multiply_h = $height / $img_size[1]; }
				else { $multiply_h = 1; }
				
				if($multiply_h > $multiply_w) { $multiply = $multiply_h; }
				else { $multiply = $multiply_w; }
