...
$.extend($[iphoneStyle].prototype, {
  ...
  wrapCheckboxWithDivs: function() {
    ...
    this.offLabel  = $('<label style="width: 18px" class="'+ this.labelOffClass +'">' +
                         '<span>'+ this.uncheckedLabel +'</span>' +
                       '</label>').appendTo(this.container);
    this.offSpan   = this.offLabel.children('span');
    
    this.onLabel   = $('<label style="width: 18px" class="'+ this.labelOnClass +'">' +
  ...
  attachEvents: function() {
    ...
    this.$elem.change(function() {
	  ...
	  if(parseInt(obj.handle.css('left')) <= 0) { new_left = new_left+5; } else { new_left = new_left-5; }
	  if(new_left == 5) { new_left = -5; }
	  if(new_left == 22) { new_left = 32; }
  ...
  initialPosition: function() {
    this.offLabel.css({ width: this.container.width() - 9 });

    var offset = ($.browser.msie && $.browser.version < 7) ? 3 : 6;
    this.rightSide = this.container.width() - this.handle.width() - offset;

    if (this.$elem.is(':checked')) {
      this.handle.css({ left: this.rightSide + 5 });
      this.onLabel.css({ width: this.rightSide + 9 });
