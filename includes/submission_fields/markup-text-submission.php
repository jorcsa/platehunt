...
<div class="_sf_box <?php echo $accordion_class; ?> _sf_box_field_<?php echo $field_id ?> <?php echo $show_cats_class; ?>" id="_sf_submission_field_box_<?php echo $field->ID; ?>">
	...
	<div class="inside">
		...
		<p class="rel" style="margin-bottom: 0;">
			...
			<input type="text" value="<?php echo $sel_val ?>" name="_sf_submission_field_<?php echo $field->ID; ?>" id="_sf_submission_field_<?php echo $field->ID; ?>" class="<?php if(get_post_meta($field->ID, 'required', true) == 'on') { echo ' required'; } ?>" style="margin-bottom: 0;" />
			<small class="error tooltip bottom" style="display: none;">Please type in a value.</small>
